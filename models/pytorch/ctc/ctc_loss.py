...
"""Connectionist Temporal Classification.
   This implementation is base on the chainer ctc implementation.
   See detail in
   https://github.com/chainer/chainer/blob/master/chainer/functions/loss/ctc.py
"""
...
def connectionist_temporal_classification(
    ...
    if input_length is None:
        ...
        input_length = variable.Variable(
            xp.full((len(x[0].data),), len(x), dtype=np.int32))
        label_length = variable.Variable(
            xp.full((len(t.data),), len(t.data[0]), dtype=np.int32))

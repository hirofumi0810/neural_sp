...
def test_forward_streaming_chunkwise(args):
    ...
    conv_lookahead = enc.conv.context_size if (enc.conv is not None and not enc.lc_bidir) else 0
    ...
    with torch.no_grad():
        for xmax in xmaxs:
            ...
            for chunk_idx in range(n_chunks):
                ...
                if enc.lc_bidir and args['streaming_type'] == 'reshape':
                    xlens_chunk = torch.IntTensor([min(xmax - j, N_c) for x in xs])
                else:

...
"""Train the hierarchical ASR model (Switchboard corpus)."""
            ...
            if epoch < config['eval_start_epoch']:
                ...
                if metric_dev < metric_dev_best:
                    ...
                    if model.main_loss_weight > 0:
                        ...
                        wer_test_swbd_sub, cer_test_swbd_sub, _ = eval_char(
                            models=[model],
                            dataset=test_data_swbd,
                            beam_width=1,
                            max_decode_len=MAX_DECODE_LEN_CHAR,
                            eval_batch_size=1)
                        logger.info(' WER / CER (%s, sub): %.3f / %.3f %%' %
                                    (test_data_swbd.data_type, wer_test_swbd_sub, cer_test_swbd_sub))
                        wer_test_ch_sub, cer_test_ch_sub, _ = eval_char(
                            models=[model],
                            dataset=test_data_ch,
                            beam_width=1,
                            max_decode_len=MAX_DECODE_LEN_CHAR,
                            eval_batch_size=1)
                        logger.info('  WER / CER (%s, sub): %.3f / %.3f %%' %
                                    (test_data_ch.data_type, wer_test_ch_sub, cer_test_ch_sub))
                        logger.info('  WER / CER (mean, sub): %.3f / %.3f %%' %
                                    (((wer_test_swbd_sub + wer_test_ch_sub) / 2),
                                     ((cer_test_swbd_sub + cer_test_ch_sub) / 2)))
                else:

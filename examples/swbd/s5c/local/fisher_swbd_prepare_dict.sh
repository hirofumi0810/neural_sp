...
dir=${data}/local/dict_nosp_swbd_fisher
mkdir -p $dir
...
for w in sil spn lau nsn; do echo $w; done > $dir/silence_phones.txt
# NOTE: make compatible with swbd
...
cat $dir/lexicon3_expand.txt  \
   <( echo "mm m"
      echo "<unk> spn" ) > $dir/lexicon4_extra.txt
...
grep -v '^#' $dir/lexicon0.txt | awk 'NF>0' | grep -v $SWB/data/dictionary/sw-ms98-dict.text | sort > $dir/lexicon1_swbd.txt || exit 1;
# NOTE: bugs in kaldi-asr

cat $dir/lexicon1_swbd.txt | awk '{ for(n=2;n<=NF;n++){ phones[$n] = 1; }} END{for (p in phones) print p;}' | \
  grep -v SIL | sort | uniq > $dir/nonsilence_phones_msu.txt  || exit 1;


local/swbd1_map_words.pl -f 1 $dir/lexicon1_swbd.txt | sort | uniq \
   > $dir/lexicon2_swbd.txt || exit 1;

cp local/MSU_single_letter.txt $dir/MSU_single_letter.txt
python local/format_acronyms_dict_fisher_swbd.py -i $dir/lexicon2_swbd.txt \

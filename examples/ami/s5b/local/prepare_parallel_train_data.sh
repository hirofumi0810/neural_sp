...
for f in $DATA_SAVEPATH/ihm/train/utt2spk $DATA_SAVEPATH/$mic/train/utt2spk; do
  if [ ! -f $f ]; then
...
mkdir -p $DATA_SAVEPATH/$mic/train_ihmdata

# the utterance-ids and speaker ids will be from the SDM or MDM data
cp $DATA_SAVEPATH/$mic/train/{spk2utt,text,utt2spk} $DATA_SAVEPATH/$mic/train_ihmdata/
# the recording-ids will be from the IHM data.
cp $DATA_SAVEPATH/ihm/train/{wav.scp,reco2file_and_channel} $DATA_SAVEPATH/$mic/train_ihmdata/
...
tmpdir=$DATA_SAVEPATH/$mic/train_ihmdata/

awk '{print $1, $1}' <$DATA_SAVEPATH/ihm/train/utt2spk | \
...
utils/apply_map.pl -f 1 $tmpdir/ihmutt2utt <$DATA_SAVEPATH/ihm/train/segments >$DATA_SAVEPATH/$mic/train_ihmdata/segments

utils/fix_data_dir.sh $DATA_SAVEPATH/$mic/train_ihmdata

...
if [ ${stage} -le 0 ] && [ ! -e ${data}/.stage_0_${datasize} ]; then
  ...
  for part in dev-clean test-clean dev-other test-other train-clean-100; do
    local/download_and_untar.sh ${data}/data $data_url $part || exit 1;
  done
  if [ ${datasize} = "460" ] || [ ${datasize} = '960' ]; then
    local/download_and_untar.sh ${data}/data $data_url train-clean-360 || exit 1;
    if [ ${datasize} = "960" ]; then
      local/download_and_untar.sh ${data}/data $data_url train-other-500 || exit 1;
    fi
  fi
  ...
  for part in dev-clean test-clean dev-other test-other train-clean-100; do
    ...
    local/data_prep.sh ${data}/data/LibriSpeech/$part ${data}/$(echo $part | sed s/-/_/g) || exit 1;
  done
  if [ ${datasize} = "100" ]; then
    ...
    if [ ${datasize} = "460" ] || [ ${datasize} = '960' ]; then
      ...
      local/data_prep.sh \
        ${data}/data/LibriSpeech/train-clean-360 ${data}/train_clean_360 || exit 1;
      ...
      if [ ${datasize} = "960" ]; then
        ...
        local/data_prep.sh \
         ${data}/data/LibriSpeech/train-other-500 ${data}/train_other_500 || exit 1;
...
if [ ${stage} -le 1 ] && [ ! -e ${data}/.stage_1_${datasize} ]; then
  ...
  if [ ${tool} = "kaldi" ]; then
    ...
    for x in ${train} dev_clean dev_other test_clean test_other; do
      steps/make_fbank.sh --nj 8 --cmd run.pl ${data}/$x ${data}/ark/log/make_fbank/$x ${data}/ark || exit 1;
      mkdir -p ${data}/feature/kaldi/${datasize}/$x
      feat-to-len scp:${data}/feature/kaldi/${datasize}/$x/feats.scp ark,t:${data}/feature/kaldi/${datasize}/$x/frame_num.scp || exit 1;
      exit 1

      if [ `echo $x | grep 'train'` ]; then
        # compute global CMVN
        compute-cmvn-stats scp:${data}/${train}/feats.scp ${data}/ark/cmvn_${train}.ark || exit 1;
        # steps/compute_cmvn_stats.sh ${data}/$x ${data}/ark/log/make_fbank/$x ${data}/ark || exit 1;

        # dump features for training
        local/dump.sh --cmd "$train_cmd" --nj 80 --do_delta $do_delta \
            ${data}/$x/feats.scp ${data}/ark/cmvn_${train}.ark ${data}/ark/log/dump_feats/$x ${data}/feature/kaldi/${datasize}/$x || exit 1;
      else

...
class AttentionMechanism(chainer.Chain):
    ...
    def __call__(self, enc_out, enc_out_a, x_lens, dec_out, aw_step):
        """Forward computation.
        Args:
            enc_out (chainer.Variable): A tensor of size
                `[B, T_in, encoder_num_units]`
            enc_out_a (chainer.Variable): A tensor of size
            ...
            aw_step (chainer.Variable): A tensor of size `[B, T_in]`
        Returns:
        ...
        for h in range(self.num_heads):
            if self.attention_type == 'content':
                ...
                #     aw_step[:, :, h: h + 1].reshape(batch_size, 1, max_time))
                conv_feat = getattr(self, 'conv_head' + str(h))(
                    aw_step.reshape(batch_size, 1, max_time))

                # For 2D conv
                # conv_feat = F.squeeze(getattr(self, 'conv_head' + str(h))(
                #     aw_step[:, :, h:h + 1].reshape(batch_size, 1, 1, max_time)), axis=2)
                # conv_feat = F.squeeze(getattr(self, 'conv_head' + str(h))(
                #     aw_step.reshape(batch_size, 1, 1, max_time)), axis=2)
                # -> `[B, out_channels, T_in]`
        ...
        for h in range(self.num_heads):
            ...
            batch_size, max_time = aw_step.shape
            context_vec = F.sum(enc_out * F.broadcast_to(
                F.expand_dims(aw_step, axis=2),
                (batch_size, max_time, enc_out.shape[-1])), axis=1, keepdims=True)

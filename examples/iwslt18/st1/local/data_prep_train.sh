...
n_en=`cat $en | wc -l`
n_de=`cat $de | wc -l`
[ $n -ne $n_en ] && echo "Warning: expected $n files, found $n_en" && exit 1;
[ $n -ne $n_de ] && echo "Warning: expected $n files, found $n_de" && exit 1;
...
[ $n_en -ne $n_de ] && echo "Warning: expected $n_en files, found $n_de" && exit 1;

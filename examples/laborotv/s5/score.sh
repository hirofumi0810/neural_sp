...
stdout=false
n_threads=1
eval_set="dev_4k dev tedx-jp-10k"

### path to save preproecssed data
data=/n/work2/inaguma/corpus/laborotv
...
for set in ${eval_set}; do
    ...
    if [ ${metric} = 'edit_distance' ]; then
        ...
        echo ${set}
        if [[ ${set} = tedx-jp-10k* ]]; then
            echo 'CER' > ${recog_dir}/RESULTS
            # add space
            paste -d " " <(cat ${recog_dir}/ref.trn.filt | cut -f 1 -d "(" | LC_ALL=en_US.UTF-8 sed -e 's/\(.\)/ \1/g') <(cat ${recog_dir}/ref.trn.filt | sed -e 's/.*\((.*)\)/\1/g') \

...
def main():
    ...
    for data_type in ['train_' + args.data_size, 'dev', 'eval1', 'eval2', 'eval3']:
        ...
        feature_save_path = mkdir_join(
            args.data_save_path, 'feature', args.tool, args.data_size, data_type.split('_')[0])
        ...
        if 'train' in data_type:
            ...
            global_mean_male = np.load(
                join(args.data_save_path, 'feature', args.tool, args.data_size, 'train/global_mean_male.npy'))
            global_std_male = np.load(
                join(args.data_save_path, 'feature', args.tool, args.data_size, 'train/global_std_male.npy'))
            global_mean_female = np.load(
                join(args.data_save_path, 'feature', args.tool, args.data_size, 'train/global_mean_female.npy'))
            global_std_female = np.load(
                join(args.data_save_path, 'feature', args.tool, args.data_size, 'train/global_std_female.npy'))

...
if [ $stage -le 0 ]; then
  ...
  find $1/data/trans -iname '*.txt'  > $tmpdir/transcripts.flist

  # for dir in fe_03_p{1,2}_sph{1,2,3,4,5,6,7}; do
  #   find $links/$dir/ -iname '*.sph'
  # done > $tmpdir/sph.flist
  find $1/audio/* -iname '*.sph' > $tmpdir/sph.flist
...
if [ $stage -le 4 ]; then
  ...
  if [ ! -s $dir/spk2gender ]; then
    ...
    cat $1/doc/fe_03_p1_filelist.tbl  $1/doc/fe_03_p2_filelist.tbl  | \

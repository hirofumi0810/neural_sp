...
[ ! -x $sph2pipe ] && echo "Could not execute the sph2pipe program at $sph2pipe" && exit 1;
...
;;'
  ...
  cat $dir/spk2utt | awk -v set=$set '{ printf("%s '${sph2pipe}' -f wav -p '${db}'/TEDLIUM_release-3/legacy/%s/sph/%s.sph |\n", $1, set, $1); }' > $dir/wav.scp

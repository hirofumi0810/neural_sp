...
def resolve_unk(str_hyp, best_hyps_sub, aw, aw_sub, idx2char):
    ...
    # Store OOV places
    for idx_oov, word in enumerate(str_hyp.split('_')):
        if word == 'OOV':
        ...
        if word == 'OOV':
            ...
            while True:
                if t_sub + backward > len(best_hyps_sub) - 1:
                    break
                elif idx2char(best_hyps_sub[t_sub + backward: t_sub + backward + 1]) != '_':
                    covered_word += idx2char(

...
dir=$CSJDATA_SAVEPATH/local/train
mkdir -p $dir
...
mkdir -p $CSJDATA_SAVEPATH/train
for f in spk2utt utt2spk wav.scp text segments; do
  cp $CSJDATA_SAVEPATH/local/train/$f $CSJDATA_SAVEPATH/train/ || exit 1;
done

echo "CSJ data preparation succeeded."

utils/fix_data_dir.sh $CSJDATA_SAVEPATH/train
